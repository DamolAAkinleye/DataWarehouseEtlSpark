...
/**
  ...
  */
object JoinTest extends BaseClass {
  ...
  override def extract(params: Params): DataFrame = {
    ...
    val a = fact.as("a").join(userDf.as("b"), expr("a.user_sk = b.user_sk"), "leftouter")
    val b = a.join(userLoginDf.as("c"), expr("a.user_login_sk = c.user_login_sk"), "leftouter")
    val c = b.join(productDf.as("d"), expr("a.product_model_sk = d.product_model_sk"), "leftouter")
    val d = c.join(promotionDf.as("e"), expr("a.promotion_sk = e.promotion_sk"), "leftouter")
    val e = d.join(appDf.as("f"), expr("a.app_version_sk = f.app_version_sk"), "leftouter")
    val f = e.join(locDf.as("g"), expr("a.web_location_sk = g.web_location_sk"), "leftouter")

    f.selectExpr("a.product_serial",
      "a.sys_ver",
      "a.wifi_mac",
      "a.app_name",
      "a.ip",
  ...
  override def load(params: Params, df: DataFrame): Unit = {
    HdfsUtil.deleteHDFSFileOrPath("/tmp/jointest")
    df.write.parquet("/tmp/jointest")
  }
}
